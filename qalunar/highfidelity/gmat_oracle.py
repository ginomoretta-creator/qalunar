...
class GmatOracleConfig:
    ...
    point_masses : tuple of str
        Bodies in the GMAT force model. ``("Earth", "Luna")`` is the
        ephemeris analogue of the CR3BP; add ``"Sun"`` for the
        four-body truth.
    ...
    point_masses: tuple[str, ...] = ("Earth", "Luna")
...
def build_schedule_script(
    ...
    dry_mass = cfg.spacecraft_mass_kg - 5.0
    report = report_name or f"qalunar_oracle_{uuid.uuid4().hex[:12]}.txt"
    points = ", ".join(cfg.point_masses)
    ...
    lines: list[str] = [
        ...
        "EPThruster.DecrementMass = false;",
...
def propagate_schedule_gmat(
    ...
    state only). The small out-of-plane components that real dynamics
    introduce are projected out (planar bridge).
    """
    cfg = oracle_config if oracle_config is not None else GmatOracleConfig()
    ...
    elapsed, x, y, _z, vx, vy, _vz = (float(v) for v in last[:7])
...
def make_gmat_truth_propagator(
    ...
    def _truth(
        ...
        return propagate_schedule_gmat(state0, t_span, schedule, sched_config, cfg)
