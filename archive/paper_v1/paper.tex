...
\begin{abstract}
% ------------------------------------------------------------------
We formulate low-thrust cislunar trajectory correction in the planar circular restricted three-body problem (CR3BP) as a quadratic unconstrained binary optimisation (QUBO) problem in which the decision variables are inherently binary engine on/off commands. This avoids the artificial quantisation error that arises when continuous pseudospectral trajectory coefficients are encoded into binary digits, complementing the continuous-coefficient QUBO transcription of De Grossi, Carbone et~al.~(2025). A first-order state-transition-matrix expansion around a reference trajectory yields a single-pass QUBO whose global minimum is reachable by exact classical (mixed-integer linear programming, brute force) and heuristic (simulated annealing) solvers, and is also expressible on D-Wave hardware. Because the linearisation is loose for long arcs, we introduce a sequential re-linearisation outer loop with a trust-region accept-or-reject step; the binary analogue of the continuous-side sequential linearisation in Carbone et~al. We also report a classical MILP baseline obtained by McCormick lifting, certified optimal up to $N{\approx}20$ decision steps, and a measured scaling sweep on the cislunar QUBO at $N$ up to $200$ that shows the \texttt{dwave-hybrid} Kerberos workflow finds lower-energy solutions than raw simulated annealing in significantly less wall time at $N \ge 100$.

A \emph{sliding-window} extension chains short single-arc QUBOs across many lunar revolutions, with an action-radius gate that suppresses scheduling far from the Moon and a velocity-only circular target that supplies a uniform braking signal in any orbital phase. The four mission phases (Earth ascent, cislunar correction, lunar capture, on-orbit station-keeping) are flown by a single \SI{100}{\kg} microsatellite using the native throttling envelope of one ion/Hall-class thruster with \SI{350}{\milli\newton} main for the spiral and the capture and \SI{5.4}{\milli\newton} for cruise and corrections. We demonstrate Phases 1\,$\to$\,2\,$\to$\,3 composed end-to-end: $\Delta v_\mathrm{total} \approx \SI{2502}{\meter\per\second}$ in $\sim$\SI{32}{\day}.
\end{abstract}
...
This section develops the QUBO formulation in four stages. We begin
with the single-arc, single-channel linearisation around the
unforced coast trajectory (Sec.~\ref{sec:method-single}), extend it
to multi-channel scheduling with an exclusion penalty
(Sec.~\ref{sec:method-multi}), close the linearisation gap on long
arcs with a sequential re-linearisation outer loop and a
trust-region accept rule (Sec.~\ref{sec:method-iterative}), and
finally chain short single-arc QUBOs across many revolutions for
lunar capture (Sec.~\ref{sec:method-sliding}).
...
final state by
\(
  \Phimat(t_i, t_f)\,\Bmat\, \uvec_i\, \Delta t,
\)
where $\Phimat(t_i, t_f) =
\Phimat(t_f, t_0)\,\Phimat(t_0, t_i)$. Defining the per-variable
...
\begin{enumerate}[leftmargin=1.5em, itemsep=1pt]
  \item[\textbf{Phase 1}] Earth ascent: an analytical Edelbaum spiral
    ...
    ($r \approx \SI{42164}{\km}$, GEO altitude) to a $r = 0.5$
...
Sec.~\ref{sec:lin-gap}--\ref{sec:nlp}--\ref{sec:multi-channel}
...
$\sim 6\,000$~km (corrected). This figure shows a $N{=}20$ run
used for the trajectory visualisation; the long-arc
linearisation-gap study of Sec.~\ref{sec:lin-gap} uses
$N{=}15$ on the same scenario and the end-to-end mission of
Sec.~\ref{sec:end-to-end} reports its Phase-2 totals separately.}
...
predicts a $142\,000$~km miss and delivers $143\,000$~km. Meanwhile the iterative solver, after
four iterations and three burns it lands within $4\,800$~km of
...
methods. The single-pass schedule has the QUBO believing the
linearised miss is small ($142\,000$~km predicted ${\approx}$
$143\,000$~km true), but the linearisation around the unforced
coast arc is so far from the actual thrust-applied dynamics at
$T = 4$ that the schedule delivers essentially what coast would.
...
\(\dot{\bvec{\lambda}} = -A(\xvec(t))^\top
\bvec{\lambda}\) backward in time from the terminal
transversality condition
\(\bvec{\lambda}(t_f) = 2\Wmat(\xvec_f - \xvec_\mathrm{target})\).
The tangential primer-vector analogue is the scalar switching
function

\begin{equation}
S(t) \;=\; -\bvec{\lambda}_v(t) \cdot \hat{\vvec}(t),
\label{eq:switching}
\end{equation}

where \(\bvec{\lambda}_v\) is the velocity sub-vector of the
...
check: \(\bvec{\lambda}\) is built from the QUBO's own
...
intent of dwave-hybrid, which decomposes the problem and runs
classical heuristics on the parts in parallel.
...
that the experiments of Sec.~\ref{sec:results} delineate.
