...
nonconvex. \emph{Left:} for three boundary-condition cases, the
feasible converged objectives of $32$ random multistarts (SLSQP);
red bars mark distinct local minima. Nearly every feasible start
reaches a different minimum, and the objective spread exceeds an
order of magnitude. \emph{Right:} primary case~A, the distribution
of feasible SLSQP minima (bars) with MATLAB \texttt{fmincon} results
from the same seeds overlaid (orange triangles). The two optimisers
scatter across the same multi-basin landscape --- \texttt{fmincon}
even reaches a minimum beyond the SLSQP range --- but coincide to
machine precision when seeded into a shared basin.}
